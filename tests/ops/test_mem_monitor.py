...
from epicure_core.ops.mem_monitor import tick


def test_tick_writes_flag_when_high(tmp_path):
    flag = tmp_path / "runner.blocked"
    with patch("epicure_core.ops.mem_monitor.psutil.virtual_memory") as mock_vm:
...
def test_gemma_runner_raises_on_block_flag(tmp_path, monkeypatch):
    ...
    with patch("epicure_core.gemma.runner.psutil.virtual_memory") as mock_vm:
