# Installing epicure-core on Raspberry Pi 5

This guide walks you through setting up **epicure-core** on a fresh Raspberry Pi 5 for use in community kitchens, test labs, or personal food exploration projects. The full bootstrap takes about **30 minutes** from power-on to "ready to code."
...
2. Create a Python venv at `~/epicure/venv`
3. Clone and build llama.cpp with ARM64 optimizations
4. Download the Gemma 4 E4B model weights (~2GB)
5. Install epicure-core in editable mode

Expected time: **20–30 minutes**. You'll see progress like:

```
[pi-bootstrap] Installing apt dependencies...
[pi-bootstrap] Creating venv at /home/pi/epicure/venv...
[pi-bootstrap] Cloning llama.cpp...
[pi-bootstrap] Building llama.cpp (this may take 10 min)...
[pi-bootstrap] Downloading Gemma 4 GGUF...
[pi-bootstrap] Installing epicure-core...
BOOTSTRAP COMPLETE
```

If the script exits with an error, read the error message (usually a missing network connection or full disk). Fix the issue and re-run `bash ~/epicure-core/ops/pi-bootstrap.sh` — it will skip completed steps.
...
1. **Activate the venv**:
   ```bash
   source ~/epicure/venv/bin/activate
   ```
   (You'll see `(venv)` in your prompt.)

2. **Check the version**:
   ```bash
   python -c "import epicure_core; print(epicure_core.__version__)"
   ```
   Should print `0.1.0.dev0`.

3. **Run the integration test**:
   ```bash
   pytest -q ~/epicure-core/tests/
   ```
...
If you plan to use `epicure-core` regularly, add the venv to your shell startup:

```bash
echo 'export PATH="$HOME/epicure/venv/bin:$PATH"' >> ~/.bashrc
source ~/.bashrc
```
...
| `ModuleNotFoundError: No module named 'epicure_core'` | Make sure the venv is activated: `source ~/epicure/venv/bin/activate`. |
...
- Join the discussion at **[github.com/epicure-suite/epicure-core](https://github.com/epicure-suite/epicure-core)**

---

**Still stuck?** Open an issue at [github.com/epicure-suite/epicure-core/issues](https://github.com/epicure-suite/epicure-core/issues) with the full output of `bash ~/epicure-core/ops/pi-bootstrap.sh`.
